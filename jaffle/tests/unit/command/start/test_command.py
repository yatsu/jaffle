...
from jaffle.config import ConfigValue, JaffleConfig
...
def test_start(command):
    with patch(
        'jaffle.command.start.command.ioloop.IOLoop.current', return_value=Mock(ioloop.IOLoop)
    ) as ioloop_current:
        with patch(
            'jaffle.command.start.command.zmq.Context.instance',
            return_value=Mock(
                socket=Mock(
                    return_value=Mock(zmq.Socket, bind_to_random_port=Mock(return_value=1234))
                )
            )
        ) as zmq_instance:
            with patch(
                'jaffle.command.start.command.zmqstream.ZMQStream',
                return_value=Mock(zmqstream.ZMQStream)
            ) as zmq_stream:
                command.start()
...
def test_shutdown(command):
    ...
    command.clients = {'client-1': Mock(), 'client-2': Mock()}
    ...
    command.procs = {'proc-1': Mock(Process), 'proc-2': Mock(Process)}
    ...
    command.kernel_connection_file_path.return_value.assert_has_calls([
        call.exists(), call.unlink(), call.exists(),
        call.unlink()
    ])
...
def test_start_sessions(command):
    ...
    client = Mock(shell_channel=Mock(get_msg=Mock(return_value={'content': {'status': 'ok'}})))
    command.kernel_manager = Mock(
        MappingKernelManager, get_kernel=Mock(return_value=Mock(client=Mock(return_value=client)))
    )

    logger = Mock()
    with patch(
        'jaffle.command.start.command.logging.getLogger', return_value=logger
    ) as get_logger:
    ...
    assert created_sessions == [{
        ...
        'kernel_name': 'my_kernel',
        'env': {
            'PYTHONSTARTUP': str(Path(__file__).parent.parent.parent.parent.parent / 'startup.py')
        }
    }]

    command.status.add_session.assert_called_once_with('sess-id', 'sess_name', 'sess_kernel')
    ...
    command.status.add_app.assert_any_call('app2', 'sess_name', 'bar.Bar', None, {})
